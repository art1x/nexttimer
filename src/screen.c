 ...
 *
 * Idle timeout is handled by NextUI; we only manage Power-button toggle.
 */
...
void screen_set_suspend_lock(bool locked) {
    /* Prevent device suspend while timer/stopwatch is running.
     * Try /sys/power/wake_lock first; silently ignore if unavailable
     * (e.g. on Linux desktop builds). */
    const char *path = locked ? "/sys/power/wake_lock"
 ...
 * replace it with our own: short press → toggle screen on/off.
 * Long press (≥ 1 s) is left to the AXP2202 PMIC hardware shutdown.
 */
